...
def ticket_exists(ticket_number: str) -> bool:
    """True if a ticket number is already in the KB (any status). Used to
    stop the demo loader from creating duplicates on a second click."""
    if not ticket_number:
        return False
    with _conn() as c:
        row = c.execute(
            "SELECT 1 FROM entries WHERE ticket_number=? LIMIT 1", (ticket_number,)
        ).fetchone()
